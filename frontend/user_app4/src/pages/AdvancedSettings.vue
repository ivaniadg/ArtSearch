...
export default {
  ...
  data() {
    ...
    const type = history.state.advancedSettings.type;
  ...
  methods: {
    precalcAdvancedSearch(){
      const formData = new FormData();
      formData.append("image_name", history.state.advancedSettings.image_name);
      ...
      axios
        .post(backend_server+"/precalcadvancedSearchQuery", formData)
        .then((response) => {
    ...
    onSubmit() {
      if (this.type == "precalc") {
        this.precalcAdvancedSearch();
      } else {
        this.customAdvancedSearch();
      }
    },
